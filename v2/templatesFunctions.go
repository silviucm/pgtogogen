...
const COMMON_CODE_FUNCTION_QUERY = `rows, err := currentDbHandle.Query(context.Background(), JoinStringParts(queryParts,""), {{range $i, $e := .Parameters}}param{{.GoFriendlyName}}{{if ne (plus1 $i) $paramCount}},{{end}} {{end}})	
